...
export default function ActiveJobsList() {
  ...
  // Filter active or in progress jobs
  const filteredActiveJobs = activeClean.filter(
    (job) => job.status === "active" || job.status === "in progress"
  );
  ...
  const renderActiveJobItem = ({ item }) => (
    <View style={styles.jobContainer}>
      ...
      <Text style={styles.jobText}>Scheduled for: {item.date} at {item.time}</Text>
      ...
      <View style={styles.cleanerContainer}>
        ...
        >
          <Text style={styles.contactButtonText}>Contact Cleaner</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <View style={styles.container}>
      {/* Active Jobs List */}
      <FlatList
        data={filteredActiveJobs} // Using the filtered active jobs
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalContainer}>
              <Text style={styles.modalTitle}>Contact {selectedCleaner.first_name} {selectedCleaner.last_name}</Text>
              <Text style={styles.modalText}>Phone: {selectedCleaner.phone}</Text>
              <Text style={styles.modalText}>Email: {selectedCleaner.email}</Text>
...
const styles = StyleSheet.create({
  ...
  jobContainer: {
    ...
    shadowOpacity: 0.3, // Faint shadow opacity
